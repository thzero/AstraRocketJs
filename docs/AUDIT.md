...
**Status: closed.** All 70 findings are fixed; the 71st (`engParser` BOM) was withdrawn as false and is
kept below with the evidence so it is not re-raised. Every fix carries a test that was checked
against the pre-fix code — reverted, re-run, and confirmed failing — so none of them is a test
written to pass. The gates that existed only on paper now run: `format:check` (unblocked by the
new root `.gitattributes` and the generated-artifact entries in `.prettierignore`), `knip`, the
Playwright suite, and the engine parity workflow.
...
**MED** ✅ FIXED | `engine/openRocketEngine.ts:682` | `assertFiniteCurve()` guards `times`/`thrusts`/`masses`, but `setMotorById` (`:730-740`) also hands the kernel `motor.diameter`, `motor.length`, `motor.cgX` and `toKernelDelay(ejectionDelay)` unchecked — and `toKernelDelay` (`:274`) passes NaN through, since `NaN >= PLUGGED_DELAY` is false. _(reported)_ | `thrustcurve.ts:146` computes `cgX: cgSamples?.[0]?.[1] ?? motor.length / 2000`; a catalog row missing `length` makes both NaN, reproducing the opaque TeaVM "number NaN cannot be converted to a BigInt" failure this guard was written to eliminate. | Extend the guard to the four scalars.
...
**MED** ✅ FIXED | `components/sim/LaunchPanel.tsx:320-345` | The four multilevel wind-layer inputs pass no `min`, while the single-level equivalents pass `minSi={0}` (`:283`, `:304`). _(reported)_ | Tick "Wind varies with altitude" and a negative wind speed, gust stddev or layer altitude reaches `simulate()` unfiltered — two code paths for one physical quantity, drifted. | Pass `min={0}` here too.
...
**MED** ✅ FIXED | `services/motorStore.ts:160-169` | The comment claims add/remove "propagate write failures", but `kv.set(...)`'s boolean is discarded. _(reported)_ | `MotorDialog.tsx:240` awaits the import, gets a clean resolve, and re-renders a catalog without the motor the user just imported. No error. | Throw on `false`.

**MED** ✅ FIXED | `services/remoteData.ts:161-189` + `services/motorDb.ts:88-94` | `fetchCatalog<T>` does no shape validation — anything that parses as JSON is returned and the fallback loop stops. _(reported)_ | A host answering `{"error":"rebuilding"}` with HTTP 200 parses fine, so the in-build fallback copy is never tried and `[...custom, ...bundled]` throws "bundled is not iterable" into the motor picker. The two-base fallback chain is defeated by a host that is up but wrong. | Take a validator (at minimum `Array.isArray`) and treat a mismatch as a base failure.
...
Worked in the order below, each step verified before the next.

**1 — Stopped the bleeding.** The design-library render loop (`onClose` out of the effect deps), a
confirmation before Delete, and the unclamped `NumRow`.

**2 — The data-loss trio.** IndexedDB writes resolved on `t.oncomplete` rather than `req.onsuccess`;
the workspace journal validated before it reaches the library; `writeIndex`'s result returned
instead of discarded.

**3 — Physics identity.** Per-component maps keyed on UUID, not `getName()`, so two parts sharing a
name are two rows; the freeform root chord computed the kernel's way (`last.x − first.x`) at every
one of the five sites.

**4 — The async guards.** Generation tokens on `runSim` and `openDesign`; the hydration gate's
writes moved inside `if (live)`; a `.catch` on the load that degrades to a working workspace and
says so in the warning banner rather than through a `setErr` the rebuild wipes on the same tick.

**5 — The engine boundary.** Finiteness guards on the four motor scalars as well as the curve; an
error envelope on `componentMasses`; shape validation in `fetchCatalog` so a bad payload fails over
to the next base instead of poisoning the cache.

**6 — Accessibility.** Escape and a focus trap on the export dialog, `role="tab"`/`aria-selected` on
the phone tab bar and settings, and distinct accessible names for every unit chip on screen — the
cross-panel "Length unit" and "Direction unit" collisions included.

**7 — Tests, everywhere they were missing.** `openRocketEngine`, `useWorkspaceEffects`,
`SettingsProvider`, `PropertyPanel`, the clipped-transition binary search, the flight-path scene,
the chart's axis extent, the hover-cursor cleanup, the export dialog's nested popover. Several
needed a seam or a small extraction first (`__setEngineForTests`, `buildLinePath`,
`buildFlightScene`, `maxFlightTime`, `useHoverCursor`) — that work is why they had no tests before.

**8 — Duplication and dead wiring.** One `download(filename, data, mime)` in `saveFile.ts` replacing
three facades with three different argument orders; one `safeFilename` replacing five sanitizers;
`fmtMb` beside `fmtNum`; the duplicated `docs/rasaero/validation/` removed in favour of the copy
next to the engine. The tube-fin and cluster validators, which had shipped with no caller, are wired
into the property panel — a collision warning on tube fins, and the cluster dropdown that `.ork`
import/export and all three views already supported but nothing could set.

**9 — The gates.** `.gitattributes` pins LF, `.prettierignore` covers the generated artifacts, and
`format:check` joined CI.
