...
vi.mock('../../src/services/saveFile', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../../src/services/saveFile')>()),
  saveBlob: (blob: Blob, name: string) => saveBlob(blob, name),
}));

const { downloadReportPdf } = await import('../../src/services/reportPdf');
