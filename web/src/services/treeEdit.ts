...
export function updateNode(tree: RocketTree, id: string, patch: Partial<ComponentNode>): RocketTree {
  ...
  for (const n of walk(next.components)) {
    if (n.id === id) { Object.assign(n, patch); break; }
  }
  return next;
}
...
export function addChild(tree: RocketTree, parentId: string, node: ComponentNode): RocketTree {
  const next = clone(tree);
  for (const n of walk(next.components)) {
    if (n.id === parentId) { (n.children ??= []).push(node); break; }
  }
  return next;
}
...
const CATALOG_TYPES: ReadonlySet<string> = new Set(['nosecone', 'bodytube', 'tubecoupler', 'centeringring', 'bulkhead', 'parachute']);
export function hasCatalog(type: string): boolean { return CATALOG_TYPES.has(type); }
...
export function catalogPatch(p: Component): Partial<ComponentNode> {
  const mat = 'materialDensity' in p && p.materialDensity
    ? { density: p.materialDensity, materialName: (p as { material?: string }).material }
    : {};
  switch (p.type) {
    case 'nosecone':
      return { shape: p.shape, length: p.length, aftRadius: p.outerDiameter / 2, ...(p.filled ? { thickness: p.outerDiameter / 2 } : {}), ...mat };
    case 'bodytube':
    case 'tubecoupler':
      return { outerRadius: p.outerDiameter / 2, length: p.length, ...(p.innerDiameter ? { thickness: Math.max(0.0001, (p.outerDiameter - p.innerDiameter) / 2) } : {}), ...mat };
    case 'centeringring':
...
export function siblingIndex(tree: RocketTree, id: string): { index: number; count: number } | null {
  const rec = (nodes: ComponentNode[]): { index: number; count: number } | null => {
    ...
    for (const n of nodes) if (n.children) { const r = rec(n.children); if (r) return r; }
    return null;
  };
...
export function moveNode(tree: RocketTree, id: string, dir: -1 | 1): RocketTree {
  ...
  const rec = (nodes: ComponentNode[]): boolean => {
    ...
    if (i >= 0) {
      const j = i + dir;
      if (j >= 0 && j < nodes.length) { const [x] = nodes.splice(i, 1); nodes.splice(j, 0, x); }
      return true;
    }
    for (const n of nodes) if (n.children && rec(n.children)) return true;
    return false;
...
export function defaultNode(type: ComponentType): ComponentNode {
  const id = newId(type);
  switch (type) {
    case 'nosecone': return { type, id, shape: 'ogive', length: 0.1, aftRadius: 0.013, thickness: 0.001 };
    case 'bodytube': return { type, id, length: 0.2, outerRadius: 0.013, thickness: 0.0005 };
 ...
 */
export function addPart(tree: RocketTree, type: ComponentType, selectedId: string | null): { tree: RocketTree; id: string } {
  ...
  if (!parentId) { const next = clone(tree); next.components.push(node); return { tree: next, id }; }
