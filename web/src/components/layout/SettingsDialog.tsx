...
export function SettingsDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => { if (!visibleTabs.some((tb) => tb.key === tab)) setTab(visibleTabs[0].key); }, [visibleTabs, tab]);
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-4">
          ...
          {tab === 'sim' && (
            <>
              <CheckRow label={t('settings.confirmDelete')} checked={settings.simulation.confirmDelete} onChange={(v) => setSim({ confirmDelete: v })} />
              ...
              <div className="pt-1 text-[11px] font-semibold uppercase tracking-wide text-slate-500">{t('settings.warnings')}</div>
              <NumRow label={t('settings.railExitMin')} unit="m/s" step={1} min={0}
                value={settings.simulation.railExitVelocityMin} onChange={(v) => setSim({ railExitVelocityMin: v ?? DEFAULT_SETTINGS.simulation.railExitVelocityMin })} />
              <NumRow label={t('settings.deploySpeedWarn')} unit="m/s" step={1} min={0}
                value={settings.simulation.deploymentSpeedWarn} onChange={(v) => setSim({ deploymentSpeedWarn: v ?? DEFAULT_SETTINGS.simulation.deploymentSpeedWarn })} />
              <div className="pt-1 text-[11px] font-semibold uppercase tracking-wide text-slate-500">{t('settings.simOptions')}</div>
              <InfoRow label={t('settings.calcMethod')} value="Extended Barrowman" />
              <InfoRow label={t('settings.simMethod')} value="6-DOF Runge-Kutta 4" />
              <NumRow label={t('settings.timeStep')} unit="s" step={0.01} min={0.001}
                value={settings.simulation.timeStep} onChange={(v) => setSim({ timeStep: v ?? DEFAULT_SETTINGS.simulation.timeStep })} />
              <NumRow label={t('settings.maxTime')} unit="s" step={60} min={1}
                value={settings.simulation.maxTime} onChange={(v) => setSim({ maxTime: v ?? DEFAULT_SETTINGS.simulation.maxTime })} />
              <NumRow label={t('settings.randomSeed')} step={1} placeholder={t('settings.seedAuto')}
                value={settings.simulation.randomSeed} onChange={(v) => setSim({ randomSeed: v })} />
            </>
          )}
        ...
        <div className="flex items-center justify-between gap-2 border-t border-white/10 p-4">
          <div className="flex gap-2">
            ...
            >{t('settings.resetTab', { name: t(TABS.find((x) => x.key === tab)!.label) })}</button>
            <button
              onClick={reset}
              className="rounded-lg bg-slate-800 px-3 py-2 text-xs font-medium text-slate-300 ring-1 ring-white/10 hover:bg-slate-700"
...
function NumRow({ label, unit, value, step, min, placeholder, onChange }: {
