  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/70 p-4">
      <div
        ...
        role="alertdialog" aria-modal="true" aria-labelledby="wip-title"
      >
        <div className="flex items-center gap-3">
          <span className="text-3xl">🚧</span>
          <h2 id="wip-title" className="text-lg font-semibold text-slate-100">{t('wip.title')}</h2>
        </div>
        <p className="mt-4 text-sm leading-relaxed text-slate-300">{t('wip.body', { name: appName() })}</p>
        <div className="mt-6 flex justify-end">
          <button
            onClick={() => update({ wipAcknowledged: true })} autoFocus
