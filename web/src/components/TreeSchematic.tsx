...
export function TreeSchematic({ tree, info, motors, onPatchNode, maxHeight = 480, selectedId, onSelect, exportData, onError, vertical, fillHeight, roll = 0, onRoll }: {
  ...
  const marginText = info && stab
    ? `${STABILITY_GLYPH[stab]} ${info.stabilityCalibers.toFixed(2)} cal · ${marginPct!.toFixed(1)}% — ${STABILITY_WORD[stab]}`
    : null;
  const cgLabel = info ? `CG · ${(info.cg * 100).toFixed(1)} cm` : 'CG';
  const cpLabel = info ? `CP · ${(info.cp * 100).toFixed(1)} cm` : 'CP';
