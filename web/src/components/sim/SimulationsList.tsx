 ...
 */
export function SimulationsList({ sims, activeId, open, onToggleOpen, onSelect, onAdd, onDuplicate, onDelete }: {
  sims: Simulation[];
  activeId: string;
  open: boolean;
  onToggleOpen: () => void;
  ...
  return (
    <section className="rounded-xl bg-slate-900 p-3 ring-1 ring-white/10">
      <div className="mb-2 flex items-center justify-between gap-2">
        <button
          onClick={onToggleOpen} aria-expanded={open}
            ...
            return (
              ...
              >
                <button onClick={() => onSelect(s.id)} className="min-w-0 flex-1 text-left">
                  <div className={`truncate text-sm font-medium ${isActive ? 'text-sky-100' : 'text-slate-200'}`}>{s.name}</div>
                ...
                <button onClick={() => onDuplicate(s.id)} title={t('sims.duplicate')} aria-label={t('sims.duplicate')} className="shrink-0 rounded px-1 text-xs text-slate-400 hover:bg-slate-700 hover:text-sky-300">⧉</button>
                {sims.length > 1 && (
