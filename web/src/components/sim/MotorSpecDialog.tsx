...
export function MotorSpecDialog({ motor, open, onClose }: { motor: MotorSpec; open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={onClose} role="dialog" aria-modal="true">
      <div className="w-full max-w-lg rounded-xl bg-slate-900 p-4 ring-1 ring-white/10" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            {motor.manufacturer && <div className="text-[11px] font-semibold uppercase tracking-wider text-amber-400/90">{motor.manufacturer}</div>}
            <h3 className="truncate text-xl font-bold text-slate-100">{motor.designation}</h3>
          </div>
          <button onClick={onClose} aria-label={t('banner.close')} className="shrink-0 rounded-md bg-slate-800 px-2 py-1 text-xs text-slate-300 hover:bg-slate-700">✕</button>
        </div>
