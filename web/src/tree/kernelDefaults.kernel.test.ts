...
const NOSE = {
  type: 'nosecone', id: 'n', shape: 'ogive', length: 0.07, aftRadius: 0.013, thickness: 0.001,
} as unknown as ComponentNode;

const TUBE = {
  type: 'bodytube', id: 'host', length: 0.2, outerRadius: 0.013, thickness: 0.0005,
} as unknown as ComponentNode;
...
const CASES: Case[] = [
  ['masscomponent.radius', n({ type: 'masscomponent', mass: 0.01, length: 0.02 }), TUBE,
    'radius', KERNEL_DEFAULTS.masscomponent.radius, 'rocket', 0.01],
  ['railbutton.outerDiameter', n({ type: 'railbutton' }), TUBE,
    'outerDiameter', KERNEL_DEFAULTS.railbutton.outerDiameter, 'component', 0.02],
  ['bodytube.thickness', n({ type: 'bodytube', length: 0.2, outerRadius: 0.013 }), null,
    'thickness', KERNEL_DEFAULTS.bodytube.thickness, 'component', 0.002],
  ['transition.thickness', n({ type: 'transition', shape: 'conical', length: 0.05, foreRadius: 0.013, aftRadius: 0.019 }), null,
    'thickness', KERNEL_DEFAULTS.transition.thickness, 'component', 0.004],
  ['transition.length', n({ type: 'transition', shape: 'conical', foreRadius: 0.013, aftRadius: 0.019 }), null,
    'length', KERNEL_DEFAULTS.transition.length, 'component', 0.09],
  ['nosecone.thickness', n({ type: 'nosecone', shape: 'ogive', length: 0.1, aftRadius: 0.013 }), null,
    'thickness', KERNEL_DEFAULTS.nosecone.thickness, 'component', 0.004],
  ['nosecone.aftRadius', n({ type: 'nosecone', shape: 'ogive', length: 0.1 }), null,
    'aftRadius', KERNEL_DEFAULTS.nosecone.aftRadius, 'component', 0.02],
  ['bulkhead.length', n({ type: 'bulkhead', outerRadius: 0.0125 }), TUBE,
    'length', KERNEL_DEFAULTS.bulkhead.length, 'component', 0.006],
  ['engineblock.thickness', n({ type: 'engineblock', length: 0.005, outerRadius: 0.0092 }), TUBE,
    'thickness', KERNEL_DEFAULTS.engineblock.thickness, 'component', 0.003],
  ['centeringring.length', n({ type: 'centeringring', outerRadius: 0.0125, innerRadius: 0.0092 }), TUBE,
    'length', KERNEL_DEFAULTS.centeringring.length, 'component', 0.006],
  ['innertube.thickness', n({ type: 'innertube', length: 0.07, outerRadius: 0.0092 }), TUBE,
    'thickness', KERNEL_DEFAULTS.innertube.thickness, 'component', 0.002],
  ['launchlug.length', n({ type: 'launchlug', outerRadius: 0.0022 }), TUBE,
    'length', KERNEL_DEFAULTS.launchlug.length, 'component', 0.09],
  ['streamer.stripLength', n({ type: 'streamer', stripWidth: 0.05 }), TUBE,
    'stripLength', KERNEL_DEFAULTS.streamer.stripLength, 'component', 0.9],
  ['podset.instanceCount',
    n({ type: 'podset', children: [{ type: 'bodytube', id: 'pb', length: 0.12, outerRadius: 0.009, thickness: 0.0005 }] }),
    TUBE, 'instanceCount', KERNEL_DEFAULTS.podset.instanceCount, 'rocket', 4],
];
