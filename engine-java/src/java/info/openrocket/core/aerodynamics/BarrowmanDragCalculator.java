...
import info.openrocket.core.aerodynamics.barrowman.ComponentAssemblyCalc;
import info.openrocket.core.aerodynamics.barrowman.FinSetCalc;
import info.openrocket.core.aerodynamics.barrowman.LaunchLugCalc;
...
public class BarrowmanDragCalculator implements DragCalculator {
	...
	private double calculateFrictionCoefficient(FlightConfiguration configuration, double mach, double Re) {
		...
		if (configuration.getRocket().isPerfectFinish()) {
			...
			if (mach > 0.9) {
				c2 = turbulentCompressibility(mach);
			}
			if (mach < 0.9) {
				Cf *= c1;
			} else if (mach < 1.1) {
	...
	private double calculatePressureCD(FlightConfiguration configuration, FlightConditions conditions,
		...
		double base = effectiveBaseCD(conditions.getMach());
	...
	// PATCH(astrarrocketjs): overridable seams for the RASAero DragCalculator
	// subclass (opt-in supersonic extensions). Base behavior is unchanged, so the
	// stock Barrowman drag calculator stays bit-identical to upstream.

	/**
	 * The base drag coefficient actually used for the aft-base and disk terms.
	 * Default: the stock Mach-only {@link #calculateBaseCD(double)}. The RASAero
	 * subclass caps it at high supersonic Mach (base-pressure vacuum limit).
	 */
	protected double effectiveBaseCD(double mach) {
		return calculateBaseCD(mach);
	}

	/**
	 * The turbulent skin-friction compressibility factor above M0.9. Default:
	 * the stock {@code 1/(1+0.15 M^2)^0.58} fit (Van Driest II to ~M4). The
	 * RASAero subclass fades to the adiabatic-wall VD-II fit at high Mach.
	 */
	protected double turbulentCompressibility(double mach) {
		return 1 / Math.pow(1 + 0.15 * pow2(mach), 0.58);
	}
	...
	protected void buildCalcMap(FlightConfiguration configuration) {
		...
		for (RocketComponent comp : configuration.getAllComponents()) {
			...
			calcMap.put(comp, createCalcObject(comp));
		}
	}
	...
	protected RocketComponentCalc createCalcObject(RocketComponent comp) {
		...
		throw new IllegalStateException(
				"No aerodynamic calculator for component: " + comp.getComponentName());
	}
}
