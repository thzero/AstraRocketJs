...
public class FreeformFinSet extends FinSet {
	 ...
	 */
	// PATCH(astrarrocketjs): parameter Point2D.Double -> Coordinate (awt-free; this
	// public overload is unused by the extracted physics). See patches/LEDGER.md.
	public void addPoint(int index, Coordinate location) throws IllegalFinPointException {
	...
	private boolean intersects(final int targetIndex) {
		...
		for (int comparisonIndex = targetIndex+1; comparisonIndex < (points.size() - 1); ++comparisonIndex) {
			...
			final double c1x = points.get(comparisonIndex).getX(), c1y = points.get(comparisonIndex).getY();
			final double c2x = points.get(comparisonIndex + 1).getX(), c2y = points.get(comparisonIndex + 1).getY();
			...
			if (Geo2D.segmentsIntersect(t1x, t1y, t2x, t2y, c1x, c1y, c2x, c2y)) {
				...
				log.warn("                   between (" + t1x + ", " + t1y + ") => (" + t2x + ", " + t2y + ")");
				log.warn("                       and (" + c1x + ", " + c1y + ") => (" + c2x + ", " + c2y + ")");
				return true;
			}
		}
		return false;
