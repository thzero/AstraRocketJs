...
public class FreeformFinSet extends FinSet {
	...
	private boolean intersects(final int targetIndex) {
		...
		final double t1x = points.get(targetIndex).x, t1y = points.get(targetIndex).y;
		final double t2x = points.get(targetIndex + 1).x, t2y = points.get(targetIndex + 1).y;
		...
		for (int comparisonIndex = targetIndex+1; comparisonIndex < (points.size() - 1); ++comparisonIndex) {
			...
			final double c1x = points.get(comparisonIndex).x, c1y = points.get(comparisonIndex).y;
			final double c2x = points.get(comparisonIndex + 1).x, c2y = points.get(comparisonIndex + 1).y;
