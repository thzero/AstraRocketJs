...
Escrito el **2026-09-22**, comparando **AstraRocketJs <AppVersion />** (motor compilado a partir de OpenRocket <UpstreamPin />) con **[ZenRockets](https://zenrockets.com)** en la versión **v0.2.0**, la última entrada de [su registro de cambios](https://zenrockets.com/docs/changelog) en esa fecha (fechada el 2026-09-15).
...
| **Una tabla de eventos que se lee de arriba abajo**, con el estado en cada instante (salida de rampa con velocidad, estabilidad, ángulo de ataque y relación empuje-peso; fin de combustión por motor; separaciones; aperturas con la velocidad a la que se dispararon; velocidad de toma de tierra) | A medias, y es presentación más que física. Los eventos se dibujan como una tira de etiquetas sobre las gráficas y unos pocos aparecen como fichas de resultado, pero no hay tabla. Casi todo lo necesario ya está calculado y presente en el resultado. |
| **Max-Q** | No disponible. La presión dinámica alcanza su máximo en algún punto del impulso y es el número que decide si el fuselaje aguanta, y nada en la aplicación lo informa. |
...
| **Imágenes de mapa bajo la trayectoria** | No disponible. El mapa del campo de lanzamiento dibuja teselas de satélite y de calles, pero la vista de traza sobre el terreno es un gráfico con el norte arriba y anillos de distancia, sin imagen debajo. |
