...
`services/launchLocationStore.ts` es el cuarto almacén de la familia (motores, materiales, plantillas de exportación y ubicaciones): un `LaunchLocationStore` tipado sobre un `KeyValueStore` bajo `pads:custom` — la clave guardada conserva a propósito el nombre antiguo de la función, porque es bajo la que alguien ya tiene sus campos guardados en el navegador y renombrarla dejaría esa entrada sin nadie que la leyera —, intercambiable mediante `setLaunchLocationStore`. Una `LaunchLocation` es un nombre más los tres campos del lugar —`latitudeDeg`, `longitudeDeg`, `launchAltitudeM`— guardados en SI como todo lo demás, y validados al guardar contra los mismos rangos que `LaunchPanel` aplica a sus campos, así que un blob editado a mano no puede meter una latitud más allá de ±90 en los términos de gravedad y Coriolis del núcleo ni en el origen de un KML.

Deliberadamente no guarda nada más. La guía, el viento y la atmósfera son condiciones del día, no propiedades de un campo.

`components/sim/LocationEditDialog.tsx` edita una ubicación por completo —nombre, latitud, longitud y altitud— y crea una desde cero, que es la única forma de añadir una desde el menú. Sus `min`/`max` son los propios rangos del almacén, así que el recorte de `NumberInput` hace inalcanzable un valor que `launchLocationStore.isLocation` rechazaría, en lugar de rechazarlo después; y su campo de altitud se enlaza al mismo ámbito de unidad del panel de lanzamiento, para que ambos se lean en la misma unidad.

`components/sim/LocationsDialog.tsx` es la lista, y es autónoma —carga del almacén y aplica una ubicación mediante `patchLaunch`—, así que el mismo componente sirve tanto al ⚙ del panel de lanzamiento como a **menú → Ubicaciones de lanzamiento**, donde no hay ningún campo de lugar en pantalla en el que escribir. `components/sim/LocationPicker.tsx` se monta en la parte superior del grupo de lugar del panel de lanzamiento y escribe a través del `onChange` / `onCommit` del propio panel, así que aplicar una ubicación es una edición normal y deshacible, sujeta a las mismas reglas de selección múltiple que teclear los números. Cuál es la ubicación «actual» se deduce COMPARANDO LOS NÚMEROS en lugar de recordar un identificador seleccionado: los campos pueden cambiar por una importación, por geolocalización o a mano, y un identificador recordado seguiría afirmando una ubicación que ya no es lo que hay en pantalla. Ambos leen la lista a través de `components/sim/useLocationList.ts`, que numera sus lecturas y descarta una respuesta ya superada: la apertura inicial de IndexedDB hace que la PRIMERA `list()` de la sesión sea la más lenta, así que una ubicación guardada mientras tanto refrescaría, se resolvería antes y luego quedaría sobrescrita por la lista vacía que aquella primera lectura tomó antes de guardar — dejando un desplegable sin ubicaciones sobre una base de datos que sí las tiene, y sin nada que reintentar porque nada sabe que está mal.

`components/sim/SiteMap.tsx` dibuja el lugar de lanzamiento, y `services/slippyMap.ts` guarda la proyección de Mercator, las dos fuentes de teselas y el cálculo del área visible. No hay biblioteca de mapas: mostrar un punto, arrastrarlo, desplazarse y acercar es todo el requisito, y Leaflet o MapLibre traerían un sistema de capas, una superficie de complementos y una hoja de estilos para lo que no usamos, así que las teselas son etiquetas `<img>` en posiciones calculadas. Mantener pura la proyección es lo que permite probarla contra cifras de Mercator calculadas a mano y no a ojo. Las dos capas vienen de Esri (`World_Imagery` y `World_Street_Map`, cuyas rutas son `{z}/{y}/{x}` en lugar del `{z}/{x}/{y}` habitual), y una regla `CacheFirst` de Workbox en `vite.config.ts` conserva hasta 1200: una tesela es una foto del terreno, así que una antigua sigue siendo correcta, y una ubicación consultada en casa tiene que dibujarse en un campo sin cobertura. Tres errores seguidos de teselas sin que ninguna haya cargado cambian el mapa por una cuadrícula de coordenadas en lugar de un recuadro gris vacío — a propósito no una línea de costa dibujada, porque inventar una aproximada pondría el marcador sobre una forma que casi es un país, y eso es peor que ninguna forma cuando lo que se trata es de decirte si los números están donde querías. Al principio la capa de calles era `tile.openstreetmap.org`, y estaba mal por dos motivos: esos servidores son infraestructura donada y sostenida por voluntarios que la Tile Usage Policy de OSM reserva para el uso de la propia OpenStreetMap, y además la hacen cumplir, así que la capa devolvió 403 en el navegador nada más publicarse. La aplicación también pedía las teselas con `referrerPolicy="no-referrer"`, que elimina la única cabecera con la que un proveedor puede saber quién le está llamando: la mitad educada de usar las teselas de otro, y justo la señal por la que bloquean. El callejero de Esri lleva datos de OSM y los acredita en la atribución, así que quienes cartografiaron las carreteras siguen citados, sobre una CDN pensada para que se use. Una prueba unitaria comprueba que ninguna URL de tesela apunta a `openstreetmap.org` y una prueba de extremo a extremo comprueba que nada llega hasta allí por la red, porque la forma fácil de «arreglar» una capa bloqueada es volver a apuntarla ahí. `components/sim/SiteMapDialog.tsx` es el mismo mapa abierto sobre el panel de lanzamiento, cuya columna es demasiado estrecha para mostrar un campo y su entorno; `LocationEditDialog` sí tiene sitio para llevarlo al lado. Ambos escriben las coordenadas por la vía de cambio de quien los llama, así que un clic en el mapa es una sola edición deshacible. Los manejadores de arrastre y de colocación por clic viven en la misma caja que contiene los botones de capa, de zoom y de centrado, así que ignoran cualquier evento de puntero que empiece en un control: sin eso, al pulsar uno el evento burbujeaba y se leía como un clic en el terreno, y cambiar de capa movía el lugar en silencio a la esquina superior izquierda del mapa, que es donde están esos botones. La captura del puntero se toma sobre la caja y no sobre `e.target`, que suele ser una tesela y se desmonta en cuanto un desplazamiento la deja atrás.

Las mismas teselas se dibujan bajo las dos vistas de resultados, es decir, tres superficies que comparten una proyección y una caché. `components/canvas/GroundTrack.tsx` las pone bajo la vista en planta y `components/canvas/FlightGroundMap.tsx` las aplica como textura al plano del suelo de la trayectoria 3D; `services/tileLayer.ts` guarda la elección entre satélite y callejero y el estado de encendido durante la sesión, así que la respuesta es la misma dondequiera que la cambies. Las dos vistas de resultados empiezan APAGADAS y no descargan nada hasta que se les pide: los anillos de distancia y la trayectoria son la medida y no dependen de una foto, así que abrir Resultados no es motivo para llamar a los servidores de teselas de otro. `SiteMap` es al revés y muestra las imágenes por defecto, porque allí la foto es la respuesta y no el contexto: existe para decirte si las coordenadas son el campo que querías. Ninguna puede usar el `SITE_ZOOM` fijo del mapa del lugar, porque ambas se ajustan al vuelo: `zoomForMetersPerPixel` elige el zoom de tesela MÁS CERCANO a la resolución que se dibuja, redondeando en espacio logarítmico y no hacia arriba, porque redondear hacia arriba deja un factor de escala en (0,5, 1] y coloca la capa de teselas con hasta el doble de lado del recuadro: cuatro veces las teselas, medidas como 49 frente a las 16 del redondeo al más cercano. La vista en planta escala después su capa por la fracción restante, de modo que la imagen se pliega a los anillos de distancia y no al revés, y `trackExtent` tiene un suelo en `MIN_EXTENT_M` (50 m, es decir, 100 m de lado) porque con aire en calma el cohete aterriza a una décima de metro de la rampa y una vista a esa escala es una foto de hierba que ningún proveedor fotografía. La capa 3D ajusta su terreno al alcance de la propia traza más un margen, en lugar del plano fijo de 60 unidades: 9 teselas para un vuelo de club típico frente a 36 para el plano entero.

Merece la pena dejar escritos dos fallos de ese trabajo, porque ambos producían algo que parecía correcto. Las teselas se piden con **CORS**: el lienzo 3D se crea con `preserveDrawingBuffer` para que la exportación de imagen pueda leerlo, y una sola textura cargada sin CORS contamina el lienzo y rompe esa exportación en silencio. Esri responde con `Access-Control-Allow-Origin: *`, pero el service worker guardaba las teselas `<img>` de los mapas 2D como respuestas OPACAS, y una respuesta opaca no puede atender una petición CORS, así que aquellas también se piden con CORS y la caché pasó a llamarse `astra-map-tiles-v2` para retirar las entradas que una versión anterior guardó opacas. En la capa 3D el `flipY` de la textura es `false`: un plano puesto horizontal girando -90° sobre x manda su propio +y a -z, así que el volteo por defecto refleja el terreno norte por sur, y una foto aérea reflejada de un campo sigue siendo una foto de un campo. `groundMapLayout` es una función pura con pruebas que comprueban que las teselas del norte caen al norte, precisamente porque una captura de pantalla no puede detectarlo. Las teselas `<img>` llevan además `max-w-none` y un tamaño CSS explícito, porque el `img { max-width: 100% }` del reset es relativo al bloque contenedor — la capa de teselas de la vista en planta es a propósito más estrecha que su recuadro cuando la imagen se amplía, y por debajo de 256px las teselas se dibujaban encogidas mientras se SEPARABAN una tesela entera, lo que se veía como costuras con ampliación moderada y como un recuadro vacío con ampliación extrema.
...
Dos reglas evitan que la biblioteca se llene de copias del mismo cohete, porque ambos fallos son indistinguibles desde la lista de Abrir y ninguno lo puede deshacer el usuario.

**Una importación se desvincula, y se nombra antes de aterrizar.** `openOrkFile` llama a `setActiveId(null)`, de modo que el siguiente guardado automático CREA una entrada: un cohete importado es su propio diseño, no una edición del que estuviera en pantalla. Lo que eso pasaba por alto era el nombre. Volver a importar un `.ork` que has estado editando en OpenRocket, o reabrir un ejemplo, producía otra entrada con el mismo nombre cada vez. `homeForImport`, en `store.ts`, resuelve ahora el conflicto **antes** de `replaceWorkspace`: sobrescribir la entrada existente, o nombrar esta otra (se propone el primer `… (2)` libre). Tiene que ocurrir antes del cambio, o el retardo de 500 ms se dispara con el diálogo abierto y crea justo la entrada por la que se está preguntando. La respuesta llega al guardado automático a través de `WorkspaceStore.setPendingName`, así que sigue habiendo exactamente un `create`, hecho por el guardado automático, en vez de que quien llama compita con él con un segundo. El diálogo es `state/promptStore.ts` + `components/common/PromptDialog.tsx`, el hermano basado en promesas de `confirmStore`: el store necesita una respuesta a mitad de una acción y no puede renderizar.

**Solo puede haber un `create` en vuelo.** `LibraryWorkspaceStore.save` comprobaba `!this.activeId` y luego esperaba a `lib.create()` antes de asignar el id, así que dos guardados solapados en esa ventana creaban una entrada cada uno y todas menos la última quedaban huérfanas: ninguna llegaba a estar activa. La ventana no es estrecha: el retardo es de 500 ms y el primer `create` en IndexedDB es la escritura más lenta de la aplicación, y el vaciado de `visibilitychange` guarda fuera del retardo por completo. Ahora los guardados solapados comparten un único `create`, y uno que se resuelva después de haberse reemplazado el espacio de trabajo no adopta su id. `DesignLibrary.create` también deshace su escritura cuando se rechaza la del puntero activo, porque un `create` a medias dejaba su diseño indexado mientras quien llamaba seguía sin id activo: una fila idéntica cada 500 ms mientras el almacenamiento siguiera diciendo que no.

No existe **Guardar** en el menú. Editar guarda automáticamente con el retardo, la descarga de la página escribe el diario, y ese elemento solo vaciaba una escritura que ya venía de camino o mandaba a Guardar como un diseño sin nombre. En su lugar, `components/layout/SaveStatus.tsx` informa de la última escritura a partir de `lastSavedAt`, que `useWorkspaceEffects` fija en la ruta de éxito del propio guardado — no donde se pidió uno —, de modo que una escritura rechazada no puede reclamar un guardado.
