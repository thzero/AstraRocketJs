...
**Map tiles are cached as you look at them.** Map imagery is the one thing the app fetches while you use it, and only for the patch of ground on screen. Three places draw it: the [launch-site map](./running-a-simulation.md#the-map), which shows it by default because there the picture is the answer, and the [ground track](./views-and-analysis.md#ground-track-after-a-simulation) and the [3D path](./views-and-analysis.md#3d-path-after-a-simulation)'s ground plane, which start bare and fetch nothing until you ask for the ground. They share one cache and mostly ask for the same ground around the same pads, so the second and third cost little beyond the first. Those tiles are then kept, so a field you checked at home draws at the launch with no signal. A place you have never viewed cannot be drawn offline: the launch-site map says so and falls back to a coordinate grid rather than a blank box, and the two results views fall back to the plain plot and bare ground plane they had before imagery — the range rings and the trajectory do not depend on the picture. Nothing about where you look is sent anywhere but the tile server itself (Esri, for both the satellite and the street layer), and the map is only fetched when you open it.
